...
import type { Study, Site, Country, StatusColorConfig } from '../types';
...
const normalizeSiteStatus = (status: string): string => {
  if (!status) return 'inactive__v';
...
// ================ 数据处理函数 ================
const processSiteData = (site: any): Site => ({
  id: site.id,
...
const processCountryData = (country: any): Omit<Country, 'siteCount' | 'activeSiteCount'> => ({
  id: country.id,
...
export const useVault = () => {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(() => 
    localStorage.getItem(SESSION_CONFIG.STORAGE_KEYS.IS_LOGGED_IN) === 'true'
  );
  const [studies, setStudies] = useState<Study[]>([]);
  const [selectedStudy, setSelectedStudy] = useState<Study | null>(null);
  const [sites, setSites] = useState<Site[]>([]);
  const [countries, setCountries] = useState<Country[]>([]);
  const [statusColors, setStatusColors] = useState<StatusColorConfig[]>([]);
  ...
  const loadStatusColors = async () => {
    try {
      const formattedColors: StatusColorConfig[] = Object.entries(STATUS_COLORS)
  ...
  // ================ 研究选择 ================
  const selectStudy = useCallback(async (study: Study) => {
    try {
      setLoading(true);
      setError(null);
